...
to compete in the University of Waterloo EV Challenge. I'm still active in the VeC Steering Council even though I have
left high school.
...
computed tomography (CECT) scans, leveraging the nnU-Net framework for segmentation and a high-resolution multitask
convolutional neural network (CNN) for joint lesion segmentation and classification. The first stage employs a
...
under challenging conditions. We introduce a data augmentation technique that simulates the presence of surgical smoke.
Combined with AutoAugment, we train a U-Net model to automatically segment instruments and the model achieves mean Dice
